// Valence - Vibrant Academy proxy as a Vercel Function, pinned to Mumbai (bom1)
// so the upstream rolexcoderz.com /VT/ sees an Indian IP (it blocks non-Indian
// IPs). Port of netlify/functions/vt.mjs; dispatch is query-param based:
//   /api/vt?f=3929       -> content page
//   /api/vt?sync=1       -> session sync (returns pdf tokens)
//   /api/vt?v=3936       -> video stream JSON
//   /api/vt?pdfurl&t=... -> pdf redirect url
//   /api/vt?pdflegacy&video_id=... -> legacy pdf
//   /api/vt?debug=1      -> raw upstream probe
...
function transform(html) {
  ...
  html = html.replace(/href="\?"/g, 'href="/api/vt"');
  html = html.replace(/href="\?batch=1&f=(\d+)[^"]*"/g, 'href="/api/vt?f=$1"');
  html = html.replace("var _S=window.location.pathname", "var _S='/api/vt'");
  html = html.replace(
    "fetch(_S+'?batch='+_B+'&_v='+_encVid(id)+'&_t='+_NK)",
    "fetch(_S+'?v='+id)");
  html = html.replace("base=_S.replace(/\\/[^\\/]*$/,'');", "base=_S;");
  html = html.replace("base = _S.replace(/\\/[^\\/]*$/, '');", "base = _S;");
  html = html.replace("base + '/api.php?action=pdfurl'", "base + '?pdfurl&'");
  html = html.replace("base+'/api.php?action=pdf'", "base+'?pdflegacy&'");
  return html;
}
...
export async function GET(req) {
  ...
  if (q.get("debug")) {
    const r = await httpGet(BATCH_URL + "&rcz_tok=" + FIXED_TOK);
    const txt = dec.decode(r.body);
    const cookieNames = [];
    if (cookieJar) cookieNames.push(...cookieJar.split(";").map((p) => p.trim().split("=")[0]));
    const hits = {};
    for (const pat of ["action=", "rcz", "code", "gate", "access", "fetch\\(", "api.php", "cookie", "token", "form", "xmlhttp"]) {
      hits[pat] = (txt.match(new RegExp(pat, "gi")) || []).length;
    }
    const codes = Array.from(txt.matchAll(/data-[a-z-]+="([^"]{4,60})"/gi), (x) => x[1]).slice(0, 10);
    return json(200, {
      status: r.status,
      server: r.resp ? r.resp.headers.get("server") : null,
      err: r.resp ? null : String(r.body).slice(0, 300),
      len: txt.length,
      cookieNames,
      hits,
      codes,
